...
public class LoginEntity {
    ...
    public boolean printOwner(String fName, String mName, String lName, String phone, String owEmail, String owUser, String owPass) throws SQLException {
        ...
        try (Connection connection = DriverManager.getConnection(url, username1, password1)) {
            ...
            statement.executeUpdate(query2);String query3 = "insert into owner (first_name, second_name, last_name, phonenumber, email, age, username, password, role) values ('" + fName + "', '" + mName + "', '" + lName + "', '" + phone + "', '" + owEmail + "', '" + age + "', '" + owUser + "', '" + owPass + "', '" + owner + "')";statement.executeUpdate(query3);statement.close();} finally {

            assert statement != null;
            statement.close();}return true;
    }
